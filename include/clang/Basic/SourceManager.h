...
#ifndef LCC_SOURCEMANAGER_H
#define LCC_SOURCEMANAGER_H
...
namespace clang {
  ...
  namespace SrcMgr {
    ...
    ///
    enum CharacteristicKind { C_User,
                              C_System };
  ...
  public:
      ...
      ContentCache(const FileEntry *Ent = nullptr)
          : Buffer(nullptr), Entry(Ent), SourceLineCache(nullptr), NumLines(0) {}
      ...
      ContentCache(const ContentCache &RHS) : Buffer(nullptr), SourceLineCache(nullptr) {
        Entry = RHS.Entry;

        assert(RHS.Buffer == nullptr && RHS.SourceLineCache == nullptr &&
  ...
  public:
      ...
      void setHasLineDirectives() { Data |= 4; }
    };
  ...
  public:
      ...
      std::pair<SourceLocation, SourceLocation> getInstantiationLocRange() const {
        return std::make_pair(getInstantiationLocStart(), getInstantiationLocEnd());
      }
