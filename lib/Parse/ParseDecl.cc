...
Parser::DeclPtrTy Parser::ParseDeclarationAfterDeclarator(Declarator &D) {
  ...
  //  DeclPtrTy ThisDecl = TemplateInfo.TemplateParams ? Actions.ActOnTemplateDeclarator(CurScope,
...
bool Parser::ParseImplicitInt(DeclSpec &DS, CXXScopeSpec *SS
  ...
  if (SS == 0) {
    ...
    if (TagName) {
      ...
      //      else
      //        ParseClassSpecifier(TagKind, Loc, DS, TemplateInfo);
      return true;
    }
  }
  ...
  while (1) {
    ...
    switch (Tok.getKind()) {
      ...
      case tok::identifier: {
        ...
        if (TypeRep == 0) {
          if (ParseImplicitInt(DS, 0)) continue;
          goto DoneWithDeclSpec;
        }
      ...
      case tok::kw_union: {
        ...
        ConsumeToken();
        ParseClassSpecifier(Kind, Loc, DS);
        continue;
      }
...
void Parser::ParseFunctionDeclarator(SourceLocation LParenLoc, Declarator &D,
  ...
  if (Tok.is(tok::r_paren)) {
    ...
    // int() -> no prototype, no '...'.
    D.AddTypeInfo(DeclaratorChunk::getFunction(/*prototype*/ false,//getLang().CPlusPlus,
                                               /*variadic*/ false,
                                               SourceLocation(),
                                               /*arglist*/ 0, 0,
