...
//    Diag(Tok, Ident ? diag::err_expected_lbrace : diag::err_expected_ident_lbrace);
//    return DeclPtrTy();
//  }
//
...
//  DeclPtrTy LinkageSpec = Actions.ActOnStartLinkageSpecification(CurScope,
...
//                   AttrList ? diag::err_expected_semi_after_attribute_list : diag::err_expected_semi_after_namespace_name, "", tok::semi);
//
...
void Parser::ParseClassSpecifier(tok::TokenKind TagTokKind,
  ...
  if (!Name && TUK != Action::TUK_Definition) {
  ...
  //      TagOrTempResult = Actions.ActOnExplicitInstantiation(CurScope,
  ...
  //
  //        SourceLocation LAngleLoc = PP.getLocForEndOfToken(TemplateInfo.TemplateLoc);
  ...
  //      TagOrTempResult = Actions.ActOnClassTemplateSpecialization(CurScope, TagType, TUK,
  //                                                                 StartLoc, SS,
  //                                                                 TemplateTy::make(TemplateId->Template),
  //                                                                 TemplateId->TemplateNameLoc,
  //                                                                 TemplateId->LAngleLoc,
...
//  DeclaratorChunk::FunctionTypeInfo &FTI = DeclaratorInfo.getTypeObject(0).Fun;
...
//
//    // function-definition:
//    if (Tok.is(tok::l_brace) || (DeclaratorInfo.isFunctionDeclarator() &&
...
//
//    if (DeclaratorInfo.isFunctionDeclarator() &&
//        DeclaratorInfo.getDeclSpec().getStorageClassSpec() != DeclSpec::SCS_typedef) {
