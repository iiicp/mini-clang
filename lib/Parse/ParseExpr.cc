...
Parser::ParseRHSOfBinaryExpression(OwningExprResult LHS, unsigned MinPrec) {
  ...
  while (1) {
    ...
    NextTokPrec = getBinOpPrecedence(Tok.getKind(), false, false
                                     /*getLang().CPlusPlus0x*/);
    ...
    if (ThisPrec < NextTokPrec ||
      ...
      NextTokPrec = getBinOpPrecedence(Tok.getKind(), false, false /*getLang().CPlusPlus0x*/);
    }
    ...
    if (!LHS.isInvalid()) {
      ...
      if (TernaryMiddle.isInvalid()) {
        ...
        if (!false && OpToken.is(tok::greatergreater))
...
Parser::OwningExprResult Parser::ParseSizeofAlignofExpression() {
  assert((Tok.is(tok::kw_sizeof) || Tok.is(tok::kw___alignof) /* || Tok.is(tok::kw_alignof)*/) &&
...
Parser::OwningExprResult Parser::ParseBuiltinPrimaryExpression() {
  ...
  switch (T) {
    default:
      assert(0 && "Not a builtin primary expression!");
    case tok::kw___builtin_va_arg: {
