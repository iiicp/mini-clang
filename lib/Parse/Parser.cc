...
Parser::Parser(Preprocessor &pp, Action &actions)
  ...
  // destructor.
  PackHandler.reset(new PragmaPackHandler(&PP.getIdentifierTable().get("pack"), actions));
  PP.AddPragmaHandler(0, PackHandler.get());

  UnusedHandler.reset(new PragmaUnusedHandler(&PP.getIdentifierTable().get("unused"), actions,
                                              *this));
  PP.AddPragmaHandler(0, UnusedHandler.get());

  WeakHandler.reset(new PragmaWeakHandler(&PP.getIdentifierTable().get("weak"), actions));
...
SourceLocation Parser::MatchRHSPunctuation(tok::TokenKind RHSTok,
  ...
  switch (RHSTok) {
    ...
    case tok::r_paren:
      LHSName = "(";
      DID = diag::err_expected_rparen;
      break;
    case tok::r_brace:
      LHSName = "{";
      DID = diag::err_expected_rbrace;
      break;
    case tok::r_square:
      LHSName = "[";
      DID = diag::err_expected_rsquare;
      break;
    case tok::greater:
      LHSName = "<";
      DID = diag::err_expected_greater;
      break;
  }
  Diag(Tok, DID);
...
void Parser::ParseTranslationUnit() {
  ...
  ExitScope();
  assert(CurScope == nullptr && "Scope imbalance!");
}
...
Parser::ParseDeclarationOrFunctionDefinition() {
  ...
  // 'extern "C"'.
  //  if (Tok.is(tok::string_literal) && false /*getLang().CPlusPlus*/ &&
...
Parser::DeclPtrTy Parser::ParseFunctionDefinition(Declarator &D) {
  ...
  //  DeclPtrTy Res = TemplateInfo.TemplateParams ? Actions.ActOnStartOfFunctionTemplateDef(CurScope,
...
//  assert((Tok.is(tok::identifier) || Tok.is(tok::coloncolon) || Tok.is(tok::kw_typename)) &&
...
//      TemplateIdAnnotation *TemplateId = static_cast<TemplateIdAnnotation *>(Tok.getAnnotationValue());
...
//      TemplateTy Template;
//      if (TemplateNameKind TNK = Actions.isTemplateName(*Tok.getIdentifierInfo(),
...
//  assert(false/*getLang().CPlusPlus*/ &&
