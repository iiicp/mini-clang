...
FileID SourceManager::createFileID(const ContentCache *File,
  ...
  if (PreallocatedID) {
    ...
    SLocEntryTable[PreallocatedID] = SLocEntry::get(Offset, FileInfo::get(IncludePos, File, FileCharacter));
...
const char *SourceManager::getCharacterData(SourceLocation SL) const {
  ...
  return getSLocEntry(LocInfo.first).getFile().getContentCache()->getBuffer()->getBufferStart() + LocInfo.second;
}
...
unsigned SourceManager::getLineNumber(FileID FID, unsigned FilePos) const {
  ...
  if (Content->SourceLineCache == nullptr)
  ...
  unsigned *Pos = std::lower_bound(SourceLineCache, SourceLineCacheEnd, QueriedFilePos);
...
SourceLocation SourceManager::getLocation(const FileEntry *SourceFile,
  ...
  return getLocForStartOfFile(Content->FirstFID).getFileLocWithOffset(FilePos + Col - 1);
}
...
bool SourceManager::isBeforeInTranslationUnit(SourceLocation LHS,
  ...
  // unit ?
  assert((LEntry != nullptr || REntry != nullptr) && "Locations in memory buffers.");
  (void) REntry;
  ...
  if (LEntry == nullptr)
    ...
    assert(REntry == nullptr && "Locations in not #included files ?");
