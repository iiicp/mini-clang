...
const FileEntry *DirectoryLookup::DoFrameworkLookup(const char *FilenameStart,
  ...
  if (SlashPos == FilenameEnd) return nullptr;
...
bool HeaderSearch::ShouldEnterIncludeFile(const FileEntry *File, bool isImport) {
  ...
  HeaderFileInfo &FileInfo_ = getFileInfo(File);

  // If this is a #import directive, check that we have not already imported
  // this header.
  if (isImport) {
    // If this has already been imported, don't import it again.
    FileInfo_.isImport = true;

    // Has this already been #import'ed or #include'd?
    if (FileInfo_.NumIncludes) return false;
  } else {
    // Otherwise, if this is a #include of a file that was previously #import'd
    // or if this is the second #include of a #pragma once file, ignore it.
    if (FileInfo_.isImport)
      return false;
  }

  // Next, check to see if the file is wrapped with #ifndef guards.  If so, and
  // if the macro that guards it is defined, we know the #include has no effect.
  if (const IdentifierInfo *ControllingMacro = FileInfo_.getControllingMacro(ExternalLookup))
