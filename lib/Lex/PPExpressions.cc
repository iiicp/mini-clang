...
static bool EvaluateValue(PPValue &Result, Token &PeekTok, DefinedTracker &DT,
  ...
  if (IdentifierInfo *II = PeekTok.getIdentifierInfo()) {
    ...
    if (!II->isStr("defined")) {
      ...
      Result.Val = 0;                 //II->getTokenID() == tok::kw_true;
  ...
  switch (PeekTok.getKind()) {
    ...
    case tok::char_constant: {// 'x'
      ...
      if (Literal.isMultiChar())
        NumBits = sizeof(int);//TI.getIntWidth();
      else if (Literal.isWide())
        NumBits = sizeof(wchar_t);//TI.getWCharWidth();
      else
        NumBits = sizeof(char);//TI.getCharWidth();
...
bool Preprocessor::
  ...
  unsigned BitWidth = sizeof(int) * 8;//getTargetInfo().getIntMaxTWidth();
