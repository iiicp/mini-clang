  ...
  switch (ResultChar) {
    ...
    case 'x': {// Hex escape.
      ...
      unsigned CharWidth = IsWide
                                   ? sizeof(wchar_t)//PP.getTargetInfo().getWCharWidth()
                                   : sizeof(char);  //PP.getTargetInfo().getCharWidth();
...
static void ProcessUCNEscape(const char *&ThisTokBuf, const char *ThisTokEnd,
  ...
  if ((UcnVal < 0xa0 &&
      ...
      || (UcnVal >= 0xD800 && UcnVal <= 0xDFFF) || (UcnVal > 0x10FFFF)) /* the maximum legal UTF32 value */ {
  ...
  if (*s == '0') {// parse radix
    ...
    if (s == ThisTokEnd) {
      ...
      PP.Diag(PP.AdvanceToTokenCharacter(TokLoc, s - begin),
              diag::err_invalid_decimal_digit)
              << std::string(s, s + 1);
      hadError = true;
      return;
    } else if (*s == '.') {
...
CharLiteralParser::CharLiteralParser(const char *begin, const char *end,
  ...
  llvm::APInt LitVal(sizeof(int) /*PP.getTargetInfo().getIntWidth()*/, 0);
...
StringLiteralParser::
  ...
  wchar_tByteWidth = ~0U;
  if (AnyWide) {
    wchar_tByteWidth = sizeof(wchar_t);//PP.getTargetInfo().getWCharWidth();
